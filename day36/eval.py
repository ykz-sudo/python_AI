...
用法说明：
- 设置MODEL_PATH环境变量或修改脚本中的默认路径（指向微调后的模型checkpoint）
- 设置EVAL_DATA_PATH指向评估数据集（jsonl格式）
- 运行脚本进行推理和评估

注意事项:
- 本脚本专门用于评估**微调后的模型**（包括LoRA/PEFT微调模型和全量SFT微调模型）
- 如果MODEL_PATH路径下含有PEFT/adapter配置文件（如adapter_config.json），
  脚本会自动识别并以微调模型加载方式进行推理和评估；否则将以全量微调模型方式评估
- 推理结果会保存到指定文件，便于后续分析
- 如需评估原始基座模型（未微调），请使用 eval2.py
"""
...
MODEL_PATH = os.environ.get('MODEL_PATH', '/root/GLM-4/finetune_demo/output/checkpoint-2500')
# 设置评估数据集路径
EVAL_DATA_PATH = os.environ.get('EVAL_DATA_PATH', '/root/GLM-4/finetune_demo/data/dev.jsonl')
# 设置结果保存路径
RESULTS_SAVE_PATH = os.environ.get('RESULTS_SAVE_PATH', '/root/evaluation_results.json')
...
def load_model_and_tokenizer(
    ...
    # 检查是否是PEFT模型（存在adapter_config.json）
    adapter_config_path = model_dir / 'adapter_config.json'
    if adapter_config_path.exists():
        # 存在adapter_config.json即为PEFT模型（如LoRA结构的微调模型）
        # 先尝试直接传递Path对象（如参考文件），如果失败则尝试其他方法
        try:
            ...
            # 如果失败，尝试使用local_files_only=True强制本地加载
        ...
        if not config_path.exists():
            raise ValueError(f"模型目录无效，未找到config.json或adapter_config.json: {model_dir}")
    ...
    # 对于tokenizer，如果是PEFT模型，tokenizer_dir可能是远程路径，需要特殊处理
    try:
        # 先尝试直接加载（支持本地路径和远程路径）
        ...
        # 如果失败，尝试检查是否是本地路径并使用local_files_only
        tokenizer_dir_path = Path(tokenizer_dir) if isinstance(tokenizer_dir, str) else tokenizer_dir
        if isinstance(tokenizer_dir_path, Path) and tokenizer_dir_path.exists():
            print(f"警告: tokenizer加载失败，尝试使用local_files_only: {e}")
            tokenizer = AutoTokenizer.from_pretrained(
                str(tokenizer_dir_path),
                trust_remote_code=trust_remote_code, 
                encode_special_tokens=True, 
                use_fast=False,
                local_files_only=True
