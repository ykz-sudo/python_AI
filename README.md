...
> ⚠️ **声明**：这是一个初学者在学习过程中整理的仓库，难免会有一些内容不准确、货不对板或者表述不当的地方。如果您发现了问题，欢迎 Issue 或 Pull Request 指正，不胜感激！

本仓库包含从 Python 基础到深度学习、大语言模型应用的完整学习路径。通过每天一个主题的方式，循序渐进地掌握 Python 编程和人工智能核心技术。

> 📝 **备注**：由于是初学者整理，部分大文件（如训练好的模型权重、数据集等）并未上传，且未经严格检查哪些文件已上传、哪些文件缺失，如有遗漏请见谅！
